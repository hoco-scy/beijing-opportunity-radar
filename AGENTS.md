...
- 先使用不会误伤资格的结构化条件。北京岗位优先处理，但所有地区每轮检查；不得把“本轮未查”写成“不符合”。安全筛选后不超过 100 个岗位时直接处理全部结果，不再为省事使用关键词继续缩小。
...
手动处理某一个已登记官方来源的积压任务时，可使用 `scope: "targeted-remediation"` 的独立运行记录。它仍须使用 `policyVersion: 6`、记录官方访问证据、筛选数量和每个岗位的匿名结论，但 `sourceChecks` 只覆盖本次实际处理的来源；不得把它伪装成全量定时扫描，也不得覆盖其他来源的最近检查状态。
