...
- 分层批处理：大型职位表用代码扫描全部行，模型只批量复核候选与疑难项，高推理不参与逐岗全表扫描。
...
岗位数据位于 `data/opportunities.json`，审核记录位于 `data/review-log.json`，来源池和运行策略位于 `data/source-registry.json`、`data/source-plan.json` 与 `data/screening-policy.json`。正文只展示通过门禁的具体岗位，`audit.html` 展示每轮匿名发现、核验、通过、未通过和继续跟踪记录。
