...
岗位数据位于 `data/opportunities.json`，审核记录位于 `data/review-log.json`，来源池和运行策略位于 `data/source-registry.json` 与 `data/source-plan.json`。正文只展示通过门禁的具体岗位，`audit.html` 展示每轮匿名发现、核验、通过、未通过和继续跟踪记录。
