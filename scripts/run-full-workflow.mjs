...
async function probeOfficialEntry(source, checkedAt) {
  const registered = [source.entryUrl, ...(source.alternateEntryUrls || [])];
  const minimumAttempts = ["critical", "active"].includes(source.tier) ? 3 : 1;
  const attemptCount = Math.max(minimumAttempts, registered.length);
  const accessEvidence = [];
  let usable = false;
  let semantic404 = false;
  for (let index = 0; index < attemptCount && !usable; index += 1) {
    const requestedUrl = registered[index % registered.length];
    try {
      const response = await fetch(requestedUrl, {
        redirect: "follow", signal: AbortSignal.timeout(12000), headers: { "user-agent": "Mozilla/5.0" }
      });
      const body = (await response.text()).slice(0, 12000);
      const isSemantic404 = /(?:页面不存在|not found|error 404|访问出错)/i.test(body);
      if (response.ok && !isSemantic404) {
        usable = true;
        accessEvidence.push({ requestedUrl, finalUrl: response.url, outcome: "page-incomplete", recipe: "公开入口健康检查已返回非错误页面；本轮未完成该来源的公告/附件筛选与逐岗核验。" });
      } else if (isSemantic404) {
        semantic404 = true;
        accessEvidence.push({ requestedUrl, outcome: "semantic-404", recipe: "公开入口返回明确的不存在或错误页。" });
      } else {
        accessEvidence.push({ requestedUrl, outcome: "network-error", recipe: `公开入口返回 HTTP ${response.status}，未取得可用于采集的页面。` });
      }
    } catch (error) {
      accessEvidence.push({ requestedUrl, outcome: "network-error", recipe: `公开入口本轮无法连接：${error?.name || "fetch-error"}。` });
    }
  }
  if (usable) return { sourceId: source.id, status: "accessible-incomplete", attempts: accessEvidence.length, checkedAt,
    note: "官方入口已可访问；本轮未能在该来源完成原生筛选、分页/附件和逐岗核验，因此不据此发布岗位。", accessEvidence };
  if (semantic404) return { sourceId: source.id, status: "semantic-404", attempts: accessEvidence.length, checkedAt,
    note: "已按登记入口与备用入口重试，未取得可用官方页面；本轮不据此判断无岗位。", accessEvidence };
  return { sourceId: source.id, status: "temporarily-unavailable", attempts: accessEvidence.length, checkedAt,
    note: "已按登记入口与备用入口重试，公开页面本轮仍不可用；本轮不据此判断无岗位。", accessEvidence };
}
...
function candidateSetHash(details) {
  return createHash("sha256").update(details.map((detail) => `${detail.officialUrl}:${fingerprint(detail)}`).sort().join("\n")).digest("hex");
}

function batchesOf(items) {
  const batchCount = Math.max(1, Math.ceil(items.length / 60));
  const batchSize = Math.ceil(items.length / batchCount);
  return Array.from({ length: batchCount }, (_, index) => items.slice(index * batchSize, (index + 1) * batchSize));
}

function reviewQueue(details, { city, sourceId }) {
  const cards = details.map((detail) => ({
    officialUrl: detail.officialUrl, fingerprint: fingerprint(detail), title: detail.title,
    organization: detail.organization, department: detail.department, location: detail.location,
    education: detail.education, publishedAt: detail.publishedAt, headcount: detail.headcount,
    responsibilities: detail.responsibilities, requirements: detail.requirements,
    hasApplyControl: detail.hasApplyControl, detailError: detail.detailError || null
  }));
  return {
    schemaVersion: 1, city, sourceId, candidateSetHash: candidateSetHash(details),
    instructions: "逐批阅读 20–60 个官网岗位卡片；根据职责、专业要求、单位业务及生物医学交叉场景作出独立判断。不要按关键词或岗位标题自动裁决。每个岗位必须写 accepted、rejected 或 deferred，并说明可公开的语义依据。",
    batches: batchesOf(cards).map((candidates, index) => ({ batch: index + 1, candidates }))
  };
}

function semanticDecisions(manifest, details, { city, sourceId }) {
  if (manifest?.schemaVersion !== 1 || manifest.city !== city || manifest.sourceId !== sourceId) {
    throw new Error("语义审核清单的城市、来源或版本不匹配；请先用 --review-queue 生成本轮官网岗位卡片并逐批完成判断。");
  }
  if (manifest.candidateSetHash !== candidateSetHash(details)) {
    throw new Error("语义审核清单不对应本轮官网候选；请重新生成队列并完成本轮的逐批判断。");
  }
  const batches = batchesOf(details);
  const batchDecisions = manifest.batchDecisions || [];
  if (batchDecisions.length !== batches.length) throw new Error("语义审核清单必须为本轮每个 20–60 条候选批次提供人工结论。");
  const overrides = new Map((manifest.overrides || []).map((decision) => [decision.officialUrl, decision]));
  if (overrides.size !== (manifest.overrides || []).length) throw new Error("语义审核清单中同一官网岗位只能出现一次。 ");
  return details.map((detail, index) => {
    const batchIndex = batches.findIndex((batch) => batch.includes(detail));
    const batchDecision = batchDecisions[batchIndex];
    if (!batchDecision || batchDecision.batch !== batchIndex + 1 || batchDecision.candidateCount !== batches[batchIndex].length) throw new Error("语义审核清单的批次编号或候选数不对应当前官网队列。");
    const decision = { ...batchDecision, ...(overrides.get(detail.officialUrl) || {}), officialUrl: detail.officialUrl, fingerprint: fingerprint(detail) };
    const required = ["officialUrl", "fingerprint", "decision", "reasonCode", "reason", "semanticBasis"];
    if (!decision || required.some((key) => !decision[key])) throw new Error(`语义审核结论字段不完整：${detail.officialUrl}`);
    if (decision.fingerprint !== fingerprint(detail)) throw new Error(`语义审核结论未对应当前官网详情：${detail.officialUrl}`);
    if (!["accepted", "rejected", "deferred"].includes(decision.decision)) throw new Error(`语义审核结论无效：${detail.officialUrl}`);
    if (decision.decision === "accepted" && (!["重点关注", "需要确认"].includes(decision.matchLevel) || !Number.isInteger(decision.priority) || decision.priority < 0 || decision.priority > 100 || !decision.matchReason)) {
      throw new Error(`拟发布岗位缺少人工判断的匹配等级、理由或优先级：${detail.officialUrl}`);
    }
    return decision;
  });
}
...
async function main() {
  ...
  const telecomSource = sources.get("chinatelecom-careers");
  const telecomDetails = await fetchChinaTelecomDetails(telecom.deduplicatedPositions);
  if (args.has("--review-queue")) {
    const queue = JSON.stringify(reviewQueue(telecomDetails, { city: recipes.city, sourceId: telecomSource.id }), null, 2);
    if (reviewQueueOutput) await writeFile(reviewQueueOutput, `${queue}\n`); else console.log(queue);
    return;
  }
  const manifest = await readJson("data/semantic-review-decisions.json");
  const decisionsByCandidate = semanticDecisions(manifest, telecomDetails, { city: recipes.city, sourceId: telecomSource.id });
  const candidateReviews = telecomDetails.map((detail, index) => {
    const result = decisionsByCandidate[index];
    return { detail, result, review: reviewRecord(detail, result, { checkedAt, sourceId: telecomSource.id }) };
  });
  const decisions = Object.fromEntries(["accepted", "rejected", "deferred"].map((decision) => [decision, candidateReviews.filter((item) => item.result.decision === decision).length]));
  const existingJobs = new Map(opportunities.jobs.filter((job) => job.sourceId !== telecomSource.id).map((job) => [job.id, job]));
  let published = 0;
  let updated = 0;
  for (const item of candidateReviews.filter((candidate) => candidate.result.decision === "accepted")) {
    const job = publishableJob(item.detail, { checkedAt, city: recipes.city, source: telecomSource, decision: item.result });
    if (existingJobs.has(job.id)) updated += 1; else published += 1;
  ...
  const sourceChecks = fullUpdate ? await Promise.all(officialIds.map(async (sourceId) => {
    const source = sources.get(sourceId);
    if (sourceId !== "chinatelecom-careers") return probeOfficialEntry(source, checkedAt);
    return {
  ...
  const run = {
    ...
    summary: targetedRemediation
      ...
      : fullUpdate
        ? `已检查 ${officialIds.length} 个官方来源；中国电信按 ${recipes.city} 官网筛选 ${telecom.deduplicatedPositions.length} 个候选并逐项读取职位详情。其余入口已记录本轮可访问性，未完成原生筛选或附件核验的来源均保留为后续处理，不据此发布岗位。`
  ...
  // Historical route-audit tests claimed complete city coverage before their
  // browser collection steps were performed. They are superseded by this
  // actual official-candidate remediation and must not remain user-facing.
